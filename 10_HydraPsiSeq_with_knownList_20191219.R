...
count_files <- "^Norm_Uprofile_"
# INDICATE PATTERN FOR REFERENCE POSITIONS FILE
pos_list_pattern <- "modList"  #actual file: Hs_rRNA_tRNA_modList.csv in exttDataDir
...
ModList_File <- list.files(extDataDir, pattern=pos_list_pattern, recursive=FALSE, full.names = TRUE)
ModList_File
...
setwd(ResultsDir)
#CREATE EMPTY FILES TO MERCGE DATA by SAMPLE
...
#assign(paste0(basename(z)),sample)  #USE OF IT?  variable has a name of SAMPLE_folder
print (head(basename(z))) 

#CALCULATIONS OF TREATMENT Table

rnames <- sample$position
row.names(sample) <- rnames		# change les "rownames" de la table sample
...
j = i+1
k = i+2
results$scoreB[i] = abs(results$NormUcount[i]-0.5*((results$NormUcount[e]*0.9+results$NormUcount[f]*1)/(0.9+1)+(results$NormUcount[j]*1+results$NormUcount[k]*0.9)/(0.9+1)))/(results$NormUcount[i]+1)}		# Correspond au score B
...
j = i+1
k = i+2
results$scoreC[i] = 1-results$NormUcount[i]/(0.5*((results$NormUcount[e]*0.9+results$NormUcount[f]*1)/(0.9+1)+(results$NormUcount[j]*1+results$NormUcount[k]*0.9)/(0.9+1)))}		# Correspond au score C
