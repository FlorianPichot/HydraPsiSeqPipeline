...
l <- i+1 
m <- i+3
results$deviation[i] = (1 - results$around[i]/(sd(results$around[j:k])+ sd(results$around[l:m])))}	# Ratio standard deviation (not needed)
...
l <- i+1 
m <- i+2
results$around2[i] = (1 - results$ratio2[i]/(mean(results$ratio2[j:k])+ mean (results$ratio2[l:m])))}		# Ratio2 normalization
...
j = i+1
k = i+2
results$scoreB[i] = abs(results$NormUcount[i]-0.5*((results$NormUcount[e]*0.9+results$NormUcount[f]*1)/(0.9+1)+(results$NormUcount[j]*1+results$NormUcount[k]*0.9)/(0.9+1)))/(results$NormUcount[i]+1)}		# Score B calculation
...
j = i+1
k = i+2
results$scoreC[i] = 1-results$NormUcount[i]/(0.5*((results$NormUcount[e]*0.9+results$NormUcount[f]*1)/(0.9+1)+(results$NormUcount[j]*1+results$NormUcount[k]*0.9)/(0.9+1)))}		# Score C calculation
