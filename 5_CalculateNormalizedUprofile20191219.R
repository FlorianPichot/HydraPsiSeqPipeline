...
for (y in type) {
Count_file    <- list.files(z, pattern=paste0(count_files,"*",y), recursive=F, full.names = TRUE) #".*" one ore more any characters
Coverage_file <- list.files(z, pattern=paste0(coverage_files,"*",y), recursive=F, full.names = TRUE)  
statRNA <-NA
...
dir.create (SampleDir, showWarnings = FALSE) #CREATE Sample FOLDER IN Preanalysis
